<template>
	<view class="changePassword">
		<view class="changePassword_title">托管服务器开通服务</view>
		...
		<view class="tishi">服务器管理员</view>
		<view class="changePassword_form">
			<view class="evan-form-show">
				<evan-form :hide-required-asterisk="hideRequiredAsterisk" ref="hosting" :model="hosting" show-message>
					<evan-form-item label="姓名" prop="name">
						<input class="form-input" type="text" placeholder-class="form-input-placeholder" v-model="hosting.name" placeholder="请输入姓名" />
					</evan-form-item>
					<evan-form-item label="手机" prop="phone">
						<input class="form-input" type="number" placeholder-class="form-input-placeholder" v-model="hosting.phone" placeholder="请输入手机号" />
					</evan-form-item>
					<evan-form-item label="办公地址" prop="address">
						<input class="form-input" placeholder-class="form-input-placeholder" v-model="hosting.address" placeholder="请输入办公地址" />
					</evan-form-item>
					...
					<evan-form-item label="托管类型" prop="hosting_type">
						<view class="item-content" @click="handleTap2()">
							<view class="item-value">
								<text class="item-label">{{ label2 }}</text>
								         <text v-if="!label2"
								           class="item-placeholder" style="color: #999;font-size: 28rpx;">请选择托管类型</text>
							</view>
						</view>
						<lb-picker
							ref="picker2"
					...
					<evan-form-item label="请输入所在部门" prop="section">
						<input class="form-input" type="text" placeholder-class="form-input-placeholder" v-model="hosting.section" placeholder="请输入所在部门" />
					</evan-form-item>
					
					<view class="tishi">服务器配置</view>
					...
					<evan-form-item label="硬盘大小" prop="ram">
						<radio-group class="radio" @change="ram">
							<label>
								...
								<radio value="500G" style="transform:scale(0.8)" /><text>500G</text>
							</label>
						</radio-group>
					</evan-form-item>
					...
					<view style="font-size: 20rpx;color: #999;">*注:是否遵守《安阳工学院虚拟服务器托管规定》</view>
				</evan-form>
			</view>
...
export default {
 ...
 submit(){
    	this.$refs.hosting.validate(async res => {
    		this.hosting.creat_time = this.$getFormatDate.getFormatDate()
    		if (res) {
    			this.active = 1
    			uni.getStorage({
    				key: 'teacher_account',
    				success: async res => {
    					this.hosting.account = res.data
    ...
    back() {
      uni.navigateBack({
		...
		cpu(e){this.hosting.cpu = e.detail.value},
		ram(e){this.hosting.ram = e.detail.value},
		rom(e){this.hosting.rom = e.detail.value},
		ios(e){this.hosting.ios = e.detail.value},
		stipulate(e){this.hosting.stipulate = e.detail.value},
  },
};
</script>
