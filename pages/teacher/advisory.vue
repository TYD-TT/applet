<template>
	<view class="advisory">
		<view class="advisory_title">VSB网站群技术咨询</view>
		<view class="tishi">各教职员工如有其他问题，请拨打现教中心信息技术科电话：2909864。</view>
		<view class="advisory_form">
			<view class="evan-form-show">
				<view class="advisory-title">咨询问题</view>
				<view class="advisory-image-box"><textarea placeholder="咨询问题" v-model="adv_msg.msg" style="width: 100%; border: 1px solid #e1e0e3;" /></view>
			</view>
			<view class="advisory-title">问题图片</view>
			<view class="advisory-image-box">
				<view class="advisory-image-item" v-for="(item, index) in imageLists" :key="index">
				...
				<view class="advisory-image-item" @click="addImage" v-if="imageLists.length < 4">
	...
	methods: {
		...
		async save() {
			uni.getStorage({
				key: 'teacher_account',
				success: async res => {
					this.adv_msg.creat_time = this.$getFormatDate.getFormatDate();
					this.adv_msg.account = res.data;
					this.active = 1;
		...
		async submit1() {
			if (this.imageLists.length == '0') {
				this.adv_msg.imgURL = this.imgURL;
				this.save();
			} else {
